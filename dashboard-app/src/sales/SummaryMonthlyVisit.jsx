...
function formatDateForDisplay(dateString) {
  ...
  // Jika format YYYY-MM-DD, konversi ke DD-MM-YYYY untuk tampilan
  const parts = trimmed.split('-');
  if (parts.length === 3 && parts[0].length === 4) {
    const [year, month, day] = parts;
    return `${day}-${month}-${year}`;
...
function parseDateFromDisplay(displayString) {
  ...
  // Jika format DD-MM-YYYY, konversi ke YYYY-MM-DD untuk internal state
  const parts = trimmed.split('-');
...
export default function ReportTableResult() {
  ...
  React.useEffect(() => {
    ...
    const startDate = formatDateForApi(filters.start_date);
    const endDate = formatDateForApi(filters.end_date);
    ...
    initialMountTimeoutId = setTimeout(() => {
      ...
      if (grid?.toolbar) {
          ...
          },
           {
             ...
             html: `
               <div class="tv-sales-toolbar-dates" title="Filter tanggal (maks 31 hari)">
                 ...
                 <div class="tv-sales-toolbar-dateinput">
                   <span class="tv-sales-toolbar-dateinput__icon" aria-hidden="true">${TOOLBAR_SVGS.calendar}</span>
                   <input id="${GRID_NAME}__start_date" type="text" placeholder="DD-MM-YYYY" maxlength="10" class="w2ui-input tv-sales-toolbar-dates__input" />
                 </div>
                 <span class="tv-sales-toolbar-dates__label">Sampai</span>
                 <div class="tv-sales-toolbar-dateinput">
                   <span class="tv-sales-toolbar-dateinput__icon" aria-hidden="true">${TOOLBAR_SVGS.calendar}</span>
                   <input id="${GRID_NAME}__end_date" type="text" placeholder="DD-MM-YYYY" maxlength="10" class="w2ui-input tv-sales-toolbar-dates__input" />
                 </div>
               </div>
             `,
           },
        ...
        grid.toolbar.on('click', (event) => {
          ...
          if (target === 'tbReset') {
            ...
            setFilters({
              ...
              wilayah: 'ALL',
              radius: 'ALL',
              start_date: toDateInputValue(firstDay),
            ...
            if (startEl) startEl.value = formatDateForDisplay(toDateInputValue(firstDay));
            const endEl = document.getElementById(`${GRID_NAME}__end_date`);
            if (endEl) endEl.value = formatDateForDisplay(toDateInputValue(now));
            return;
          }
        ...
        setTimeout(() => {
          ...
          const startEl = document.getElementById(`${GRID_NAME}__start_date`);
          if (startEl) startEl.value = formatDateForDisplay(filters.start_date ?? '');
          const endEl = document.getElementById(`${GRID_NAME}__end_date`);
          if (endEl) endEl.value = formatDateForDisplay(filters.end_date ?? '');

          attach(`${GRID_NAME}__query`, 'input', (e) => {
            setFilters((prev) => ({ ...prev, query: e.target.value }));
          });

          // Auto-format saat mengetik tanggal
          ...
          attach(`${GRID_NAME}__start_date`, 'input', (e) => {
            autoFormatDate(e);
          });

          attach(`${GRID_NAME}__start_date`, 'blur', (e) => {
            const value = e.target.value;
            if (value && value.length === 10) {
              setFilters((prev) => ({ ...prev, start_date: parseDateFromDisplay(value) }));
            } else if (!value) {
              setFilters((prev) => ({ ...prev, start_date: '' }));
            }
          });

          attach(`${GRID_NAME}__end_date`, 'input', (e) => {
            autoFormatDate(e);
          });

          attach(`${GRID_NAME}__end_date`, 'blur', (e) => {
            const value = e.target.value;
            if (value && value.length === 10) {
              setFilters((prev) => ({ ...prev, end_date: parseDateFromDisplay(value) }));
            } else if (!value) {
  ...
  React.useEffect(() => {
    ...
    const startEl = document.getElementById(`${GRID_NAME}__start_date`);
    const nextStart = formatDateForDisplay(filters.start_date ?? '');
    if (startEl && startEl.value !== nextStart) startEl.value = nextStart;

    const endEl = document.getElementById(`${GRID_NAME}__end_date`);
    const nextEnd = formatDateForDisplay(filters.end_date ?? '');
    ...
    >
      ...
      <style>
        {`
          ...
          .tv-sales-toolbar-dateinput {
            position: relative;
            display: inline-flex;
            align-items: center;
          }
          .tv-sales-toolbar-dateinput__icon {
            position: absolute;
            right: 8px;
            top: 50%;
          ...
          .tv-sales-toolbar-dateinput__icon svg {
            display: block;
            width: 14px;
            height: 14px;
            color: #8d99a7;
          }
          .tv-sales-toolbar-dateinput .tv-sales-toolbar-dates__input {
            padding-right: 26px;
          }
